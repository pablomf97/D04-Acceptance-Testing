...
public interface ProviderRepository extends JpaRepository<Provider, Integer> {
	...
	//select p.name,(1.0*(select count(5) from Item i where i.provider=p)) from Provider p;
	// select p.name,(1.0*(select count(*) from Item i where i.provider=p)) from Provider p order by p;
	//select p.name from Provider p group by (1.0*(select count(5) from Item i where i.provider=p));
	...
	// select p.name,(1.0*(select count(*) from Item i where i.provider=p)) from Provider p order by ((1.0*(select count(*) from Item i where i.provider=p))) desc;
